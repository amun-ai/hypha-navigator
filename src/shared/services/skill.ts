...
export function generateSkillMd(
  ...
  const frontmatter = [
    "---",
    "name: web-navigator",
    "description: Hypha Navigator — AI browser automation over Hypha RPC. Drive tabs (open/close/switch/navigate) and remotely inspect & control the target page in a real browser: read the DOM, take screenshots, run JavaScript, fill forms, click/type by index, inspect React — all via HTTP API calls.",
    "compatibility: Requires network access to the Hypha server. Works with any HTTP client (curl, fetch, Python requests).",
    "metadata:",
    '  version: "0.1"',
    '  author: "hypha-navigator"',
    "---",
  ].join("\n");
  ...
  const intro = [
    "",
    "# Hypha Navigator — drive a browser & control its pages",
    "",
    "**What this is:** Hypha Navigator is connected to a real Chrome browser, and this document is its HTTP API. You (an AI agent) can drive the whole browser — open/close/switch/navigate tabs — and remotely inspect and control the current target page: read and modify the DOM, click and type, take screenshots, run JavaScript, and inspect React components. Changes you make are applied to the real page immediately.",
    "",
    attached,
    "",
    "## Fast path — the `hyd` CLI (recommended for agents)",
    "",
    "For repeated work, don't hand-write authenticated curl calls. Install the `hyd`",
    "CLI once (it ships in the `hypha-debugger` pip package) and drive this browser with",
    "tiny commands — it stores this connection as a **browser profile**, so later calls",
    "carry no auth or JSON boilerplate:",
    "",
    "```bash",
    "pip install hypha-debugger            # ships the `hyd` command",
    "hyd profile add web \"{SERVICE_URL}\" --type browser   # add --token <t> if this URL is protected",
    "export HYD_PROFILE=web                # select it (env var — per-terminal, not on disk)",
    "",
    "hyd 'document.title'                 # bare form on a browser profile runs JavaScript (execute_script)",
    "hyd js 'await fetch(\"/api/data\").then(r=>r.json())'   # explicit JS; returns the result",
    "hyd nav 'https://example.com'        # navigate the page",
    "hyd shot page.png                    # save a screenshot to a PNG file",
    "hyd call get_browser_state           # call ANY function below by name",
    "hyd call click_element_by_index --json '{\"index\":2}'",
    "```",
    "",
    "Use it whenever you'll run more than one command here — far fewer tokens than curl,",
    "and it behaves like a local tool. The SAME `hyd` also manages terminal (Python",
    "process) profiles, so one CLI drives both; run `hyd` with no args for help. Install tip:",
    "`pipx install hypha-debugger` always puts `hyd` on PATH; if a `pip --user` install leaves",
    "`hyd` unfound, use `python -m hypha_debugger.cli` instead (same args).",
    "",
    "**No install? Use `curl` — everything works without the CLI.** If you can't install a",
    "pip package (no pip/network, locked-down sandbox), skip `hyd` entirely: the raw HTTP API",
    "below is complete on its own, with copy-paste `curl` examples for every function (it's",
    "exactly what `hyd` calls under the hood). Recommended path is `hyd`; guaranteed-available",
    "path is `curl`. (Replace `{SERVICE_URL}` with this service's URL — the one you fetched",
    "this document from, minus `/get_skill_md`.)",
    "",
    ...
    "",
    "- `{success: false, detail: \"...\"}` — e.g. service not found, call timed out, disconnected. If you see this, the browser tab is probably closed or Hypha Navigator disconnected.",
    "",
  ].join("\n");
  ...
  const tips = [
    ...
    "- **Use `navigate`** to send the target tab to another URL, or `open_tab`/`activate_tab` to control which tab is the target.",
    "- **If you get `{success: false, detail: \"Service not found\"}`** — the browser tab was closed or Hypha Navigator disconnected. The user needs to re-connect from the Hypha Navigator side panel.",
