...
export function generateSkillMd(
  ...
  const intro = [
    ...
    "process) profiles, so one CLI drives both; run `hyd` with no args for help. If `hyd`",
    "isn't on your PATH after install, use `python -m hypha_debugger.cli` instead (same",
    "args). The raw HTTP API below still works and powers the CLI. (Replace `{SERVICE_URL}`",
