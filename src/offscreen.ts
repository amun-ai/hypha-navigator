...
const SKILL_GUIDANCE = [
  ...
  "",
  "6. **Turn reusable operations into SITE TOOLS — named, callable scripts (bound to origin).**",
  "   Where a *skill* is markdown know-how, a *tool* is EXECUTABLE: define it once, then CALL it",
  "   by name with arguments instead of re-sending a full execute_script each time. Operation",
  "   results carry a `site_tools` signature index and a `site_tools_hint` — read and act on them.",
  "   - Define/mutate: `set_site_tool(origin, name, description, params, code)` — `params` is an",
  "     array of `{name, type?, description?, required?, default?}`; `code` is a JS body where the",
  "     call arguments are in scope as an `args` object and the LAST expression is auto-returned",
  "     (async/await + `fetch` supported). Save over the same name to update it.",
  "   - Call (fast path): `call_site_tool(origin, name, args)` → runs the stored code with your",
  "     `args` injected; returns `{result, type}` like execute_script — no script re-sent.",
  "   - `list_site_tools(origin?)` shows name+description+params (not the code); `get_site_tool`",
  "     reads the code; `remove_site_tool` prunes. A skill's markdown can reference tool names.",
  "   Rule of thumb: once you've run the same execute_script twice, capture it as a site tool.",
  "",
  "**Loop:** check site_skills + site_tools on the result → (explore once if new) → prefer",
  "call_site_tool, else script via execute_script & batch → set_site_skill / set_site_tool what",
  "you learned. Each origin gets faster and cheaper over time.",
  "",
