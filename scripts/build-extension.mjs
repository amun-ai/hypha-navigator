/**
 * Build the Hypha Navigator MV3 extension with esbuild. Bundles each entry,
 * post-processes the hypha-rpc bundle for the extension CSP, and copies the
 * static files into ./dist/. Load that dir unpacked in chrome://extensions.
 *
...
try {
  ...
  const docs = p("docs", "index.html");
  if (existsSync(docs)) {
    let html = readFileSync(docs, "utf8");
    html = html
      .replace(
        /(href="hypha-navigator-extension\.zip\?v=)[^"]*(")/,
        `$1${version}$2`,
      )
      .replace(
